...
def MakeFourierPlotEvenOdd():
	...
	ax = subplot(2, 1, 1)
	f1 = fft.fft(d1)[:N1/2]
	for dE in E[2:] - E[:-2]: ax.axvline(dE, color=lineColor, linestyle="--", alpha="0.5")
	for dE in diff(E): ax.axvline(dE, color=lineColor, linestyle=":", alpha="0.5")
...
def DrawBigClock(theta, drawLines=True, drawTicks=True):
	...
	if drawLines:
		lin1 = matplotlib.patches.Line2D(xdata=[-radius,radius], ydata=[0,0], color="#aaaaaa")
		ax.add_artist(lin1)
		lin2 = matplotlib.patches.Line2D(xdata=[0, 0], ydata=[-radius, radius], color="#aaaaaa")
		ax.add_artist(lin2)
	...
	lin3 = matplotlib.patches.Line2D(xdata=[0,dx], ydata=[0,dy], color="b")
	ax.add_artist(lin3)
...
def CorrelationBarPlotPhaseClockFig(corr, corr2, ax=None, axisHeight=None):
	...
	for i, curCorr in enumerate(corr[:-1]):
		if abs(corr[i])**2 > 1e-2:
			...
			lin = matplotlib.patches.Line2D(xdata=[xCenter1, xCenter1+dx], ydata=[yCenter1, yCenter1+dy])
			ax.add_artist(lin)
			...
			lin = matplotlib.patches.Line2D(xdata=[xCenter2, xCenter2+dx], ydata=[yCenter2, yCenter2+dy])
...
def MakeCouplingFig(**args):
	...
	cmap = LoadColormap("gradient.txt", True)
	imshow(matrix[:8,:8], interpolation="nearest", cmap=cmap)
	colorbar()
