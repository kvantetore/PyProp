def RemoveBoundStateProjection(psi, boundStates):
	...
	for b in boundStates:
		tmpPsi.GetData()[:] = b[:]
		curProj = psi.InnerProduct(tmpPsi)
		psi.GetData()[:] -= curProj * tmpPsi.GetData()[:]
...
def RunGetSingleIonizationProbability(fileList, removeBoundStates=True):
	...
	singleIonEnergies, singleIonStates = GetFilteredSingleParticleStates(isIonized, config=conf)
	singleBoundEnergies, singleBoundStates = GetFilteredSingleParticleStates(isBound, config=conf)
