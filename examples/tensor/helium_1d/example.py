...
def Propagate(saveWavefunction = False, **args):
	...
	outputFilename = "output/convergence/%s/propagation_krylov_%s_%s_%s_T_%s_dt_%1.2f" % ("_".join(pulsepostfix), krylovSize, "_".join(gridpostfix), "_".join(pulsepostfix), T, dt)
...
def GetGroundstateWavefunction(**args):
	groundstateFilename = GroundstateFilename(**args)
	h5file = tables.openFile(groundstateFilename, "r")
	cfgObj = h5file.root.wavefunction._v_attrs.cfgObj
	try:
		groundstate = h5file.root.wavefunction[:]
	finally:
		h5file.close()
...
def SaveGroundstate(**args):
	...
	args["eigenvalueCount"] = getattr(args, "eigenvalueCount", 2)
	args["eigenvalueBasisSize"] = getattr(args, "eigenvalueBasisSize", 10)
...
#------------------------------------------------------------------------------------
def ReconstructRadialDensityOnGrid(psiFile, xGrid):
	"""
	Construct grid density for a 2D wavefunction.
	"""

	#Create the wavefunction
	psi = pyprop.CreateWavefunctionFromFile(psiFile, "/wavefunction")

	#Total number of B-splines
	bsplineObject = psi.GetRepresentation().GetRepresentation(0).GetBSplineObject()
	numBsplines = bsplineObject.NumberOfBSplines

	#Array for final grid density
	gridDensity = zeros((xGrid.size, xGrid.size), dtype=double)

	#A buffer for reconstructing 
	buffer1D = zeros(xGrid.size, dtype=complex)
	buffer2D = zeros((numBsplines, xGrid.size), dtype=complex)
	psiSliceGrid = zeros(xGrid.size, dtype=complex)
	psiSlice = zeros(numBsplines, dtype=complex)

	#Calculate grid density
	for i in range(numBsplines):
		psiSlice[:] = psi.GetData()[i, :]
		buffer1D[:] = 0.0
		bsplineObject.ConstructFunctionFromBSplineExpansion(psiSlice, xGrid, buffer1D)
		buffer2D[i,:] = buffer1D[:]
