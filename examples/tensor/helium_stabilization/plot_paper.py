...
def PaperMakePlotDpDomegaScan(experiment, doClose=True):
	e0list = [1,5,10,15,20]
	E1 = (5 - PaperGetBaseEnergy(experiment))/2
	E2 = (2*5 - PaperGetBaseEnergy(experiment))/2
	...
	try:
		for e0 in e0list:
			fig = PaperMakePlotDpDomega(experiment, e0, E1, E1)
			fig.savefig(os.path.join(folder, "%s_dpdomega_e0_%i_1photon_equalenergy.eps" % (experiment, e0,)), dpi=300)
			fig.savefig(os.path.join(folder, "%s_dpdomega_e0_%i_1photon_equalenergy.pdf" % (experiment, e0,)), dpi=300)
			if doClose: close(fig)
		
			fig = PaperMakePlotDpDomega(experiment, e0, E2, E2)
			fig.savefig(os.path.join(folder, "%s_dpdomega_e0_%i_2photon_equalenergy.eps" % (experiment, e0,)), dpi=300)
			fig.savefig(os.path.join(folder, "%s_dpdomega_e0_%i_2photon_equalenergy.pdf" % (experiment, e0,)), dpi=300)
			if doClose: close(fig)
