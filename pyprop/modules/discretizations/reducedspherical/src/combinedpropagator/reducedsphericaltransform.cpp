...
void ReducedSphericalTransform<Rank>::SetupStep(const Wavefunction<Rank> &psi, int baseRank)
{
	...
	typename CmbRepr::Ptr reprComb = pyprop_dynamic_cast< CmbRepr >(psi.GetRepresentation());
	SphHarmRepr::Ptr reprAngular = pyprop_dynamic_cast< SphHarmRepr >(reprComb->GetRepresentation(sphRank));
	if (reprAngular == 0) 
	{
