...
from pyprop.tensorpotential.tensorpotentialgenerator import \
...
#                           Spherical Harmonic
#-------------------------------------------------------------------------------
class GeometryInfoSphericalHarmonic(geometryinfo.GeometryInfoBase):
	"""
	Geometry information for spherical harmonic geometries
	The selection SphericalHarmonicBasisRepresentation and a
	SphericalSelectionRule is specified.
...
class GeometryInfoSphericalHarmonicDistributed(geometryinfo.GeometryInfoDistributedBase):
	"""
	Geometry information for spherical harmonic geometries using distributed matvec.
	The SphericalHarmonicBasisRepresentation and a
	SphericalSelectionRule is specified.
...
class BasisfunctionSphericalHarmonic(geometryinfo.BasisfunctionBase):
	...
	The SphericalHarmonicBasis representation is never represented
	...
	def SetupBasis(self, basisRepresentation):
		...
		self.GeometryFunction = GeometryInfoSphericalHarmonic
		if distr.IsDistributedRank(baseRank):
			self.GeometryFunction = GeometryInfoSphericalHarmonicDistributed

	def GetGridRepresentation(self):
		raise Exception("Spherical Harmonics Basis does not support grid representations")
	...
	def RepresentPotentialInBasis(self, source, dest, rank, geometryInfo, \
		...
		raise Exception("Spherical Harmonics are already in a basis and should not be integrated")
