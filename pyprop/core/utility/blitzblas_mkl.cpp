...
#define BLAS_NAME(name) mkl::name
...
void MatrixMatrixMultiply(const Array<cplx, 2> &A, const Array<cplx, 2> &B, Array<cplx, 2> &C)
{
	...
	*/
	...
	BLAS_NAME(zgemm)(
		...
		(mkl::doublecomplex*)B.data(),	// Pointer to A
		ldb,				// Size of first dim of A
		(mkl::doublecomplex*)A.data(),	// Pointer to B
		lda,				// Size of first dim of B
		&beta,				// Scaling of C
		(mkl::doublecomplex*)C.data(),	// Pointer to C
...
void MatrixVectorMultiply(const Array<cplx, 2> &A, const Array<cplx, 1> &v, Array<cplx, 1> &w)
{
	...
	BLAS_NAME(zgemv)(
		...
		N, 					// Cols of A
		&alpha, 				// Scale of A*v
		(mkl::doublecomplex*)A.data(), 				// Pointer to A
		lda, 					// Size of first dim of A
		(mkl::doublecomplex*)v.data(),				// Pointer to input vector 
		vStride, 				// Stride of input vector
		&beta,	 				// Scale of w
		(mkl::doublecomplex*)w.data(), 				// Pointer to output vector
...
cplx VectorInnerProduct(const blitz::Array<cplx, Rank> &u, const blitz::Array<cplx, Rank> &v)
{
	...
	cplx result;
	mkl::doublecomplex ret = BLAS_NAME(zdotc)(N, (mkl::doublecomplex*)u.data(), uStride, (mkl::doublecomplex*)v.data(), vStride);
